...
전체 설계는 PROJECT_PLAN.md, 하네스 규칙(레이어 의존성·금지사항)은 AGENTS.md 참고 — 먼저 읽고 시작해.
tests/test_architecture.py가 AGENTS.md 규칙을 ast로 강제하니 모든 단계에서 통과 상태를 유지해.
...
- tests/test_architecture.py(기존): score.py/cli.py가 생기면 skip이 풀리며 순수성·레이어 규칙을 자동 검사한다.
  구현 후 pytest tests/ 전체가 green이어야 완료.

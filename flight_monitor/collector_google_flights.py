...
async def _fetch_airport(
    ...
    async with semaphore:
        ...
        # 이번 tick이 새로 저장한 레그가 save_legs로 DB에 반영된 뒤 호출되므로,
        # 슬라이싱으로 out/in 레그가 여러 run에 흩어져도 왕복쌍 누락 없이 deals가 완성된다.
        if on_route_done:
            loop = asyncio.get_event_loop()
            await loop.run_in_executor(None, on_route_done, "google_flights", airport_code, airport_name)
...
async def _fetch_all(on_route_done=None) -> list[dict]:
    ...
    # 이번 tick이 담당할 개월 슬라이스만 수집 (sweep 분산). tick_months >= range_months면
    # 전체 범위(기존 동작). end_date는 복귀편까지 담도록 max_stay만큼 연장됨.
    today = date.today()
    ...
    print(f"[GoogleFlights] sweep 윈도우 {start_date} ~ {end_date} (tick_months={tick_months})")
